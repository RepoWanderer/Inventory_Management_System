...
- The screen shows the options for `Employee`, `Supplier`, `Category`, `inventory, 'billing',  and "Sales" to perform CRUD operations.
...
- You have to run this file `first` before running the `GUI.py` file otherwise it will throw an error.
...
#### Detailed Steps:
1. Click on the `DB.py` file first and run it.
2. Click on the `GUI.py` file and run it.
...
7. Now click on the `GUI.py` file and run it.
...
### Configuration
- Before running `GUI.py`, make sure to run `DB.py` file first and create a folder named `bill` and `images`.
